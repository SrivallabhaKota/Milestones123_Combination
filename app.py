...
def add_budget():
    ...
    return render_template('budget.html', error=error, user_name=session.get('user_name', 'User'), active_goals=active_goals)
...
def profile_page():
    ...
    try:
        ...
        total_saved = total_invested = 0

    goal_stats = {

        'total': total_goals, 'active': active_goals,
    ...
    try:
        ...
        with conn.cursor() as cur:
                    ...
                    if not cur.fetchone():
                        ...
                        cur.execute(
                            ...
                            (user_id, 'budget_exceeded',
                             ...
                             f'You have spent ₹{spent:,.0f} out of ₹{limit_amt:,.0f} limit ({pct:.0f}%). Immediate action needed.',
                    ...
                    if not cur.fetchone():
                        ...
                        cur.execute(
                            ...
                            (user_id, 'budget_warning',
                             ...
                             f'You have used {pct:.0f}% of your ₹{limit_amt:,.0f} budget. ₹{limit_amt-spent:,.0f} remaining.',
            ...
            cur.execute(
                ...
                "SELECT id, goal_name, target_date, current_amount, target_amount FROM goals WHERE user_id=%s AND status='Active' AND target_date IS NOT NULL",
            ...
            for g in goals:
                ...
                if 0 < days_left <= 7:
                    ...
                    cur.execute(
                        ...
                        "SELECT id FROM alerts WHERE user_id=%s AND trigger_ref=%s AND alert_type='goal_critical' AND MONTH(created_at)=%s AND YEAR(created_at)=%s",
                    ...
                    if not cur.fetchone():
                        ...
                        cur.execute(
                            ...
                            (user_id, 'goal_critical',

                             f'🔴 Goal Deadline in {days_left} day(s): {g["goal_name"]}',

                             f'Only {days_left} day(s) left! You still need ₹{remaining:,.0f} to complete this goal.',
                    ...
                    if not cur.fetchone():
                        ...
                        cur.execute(
                            ...
                            (user_id, 'goal_deadline',
                             ...
                             f'{days_left} days remaining. Need ₹{remaining:,.0f} more. Stay on track!',

                             'warning', ref)

                        )

            # ── 3. Weekly spending spike alert ───────────────────────

            cur.execute(

                "SELECT COALESCE(SUM(amount),0) AS total FROM expenses WHERE user_id=%s AND expense_date >= %s",

                (user_id, (today - timedelta(days=7)).strftime('%Y-%m-%d'))

            )

            this_week = float(cur.fetchone()['total'])
            ...
            if last_week > 0 and this_week > last_week * 2:
                ...
                if not cur.fetchone():
                    ...
                    cur.execute(
                        ...
                        (user_id, 'weekly_spike',
                         ...
                         f'📈 Weekly Spending Spike Detected!',
